...
const GOV_COORDS = {
  ...
  "طرطوس": [34.8950, 35.8866],
  "السويداء": [32.6989, 36.5683],
  "درعا": [32.6258, 36.1054],
  "القنيطرة": [33.1258, 35.8245],
  "ادلب": [35.9306, 36.6339],
  "الرقة": [35.9606, 39.0085],
  "دير الزور": [35.3356, 40.1404],
...
function getGovIntensity(gov, arr){
  ...
  govData.forEach(d => {
    total += (d.population||0) * 0.5 + (d.males||0) * 0.1 + (d.females||0) * 0.1 +
             ...
             (d.disabled_cards||0) * 2 + (d.elderly_benef||0) * 1.5 +
             (d.ngo_active||0) * 50 + (d.juvenile_inmates||0) * 2 +
             (d.rural_dev_benef||0) * 0.5 + (d.rural_ind_benef||0) * 0.5 +
             ((d.insp_industrial !== undefined ? d.insp_industrial : (d.insp_total||0)) + (d.insp_agricultural||0)) * 0.5 +
             (d.new_workers_insp||0) * 0.5 + (d.violations||0) * 2 +
             (d.platform_registered||0) * 0.1 + (d.vocational_benef||0) * 0.5 +
             (d.food_baskets||0) * 0.5 + (d.cash_assistance||0) * 0.5 +
             (d.camps_count||0) * 100 + (d.insurance_total||0) * 0.1;
  });
...
function getIntensityColor(intensity){
  ...
  if(intensity > 5000) return '#B5533C';
  if(intensity > 2000) return '#B08D57';
  if(intensity > 500) return '#4F7A63';
  return '#6E8FA3';
}
...
function initMap(){
  ...
  if(!mapEl) return;
  map = L.map('govMap').setView([34.8, 38.0], 6);
...
function updateMapMarkers(){
  ...
  ALL_GOVS.forEach(gov => {
    ...
    const customIcon = L.divIcon({
      ...
      html: `<div style="width:28px;height:28px;background:${color};border:3px solid white;border-radius:50%;box-shadow:0 2px 8px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:11px;font-family:'IBM Plex Mono',monospace;">${gov[0]}</div>`,
      iconSize: [28, 28],
      iconAnchor: [14, 14]
    });
    const marker = L.marker(coords, { icon: customIcon }).addTo(map);
    marker.on('click', function(){ openGovProfile(gov); });
    let popupContent = `<div class="map-info-window"><h4>📍 ${gov}</h4>`;
    if(govData.length === 0){
      ...
      const d = agg;
      const monthsList = [...new Set(govData.map(d => d.month).filter(Boolean))].join('، ');
      popupContent += `
        ...
        <div style="border-top:2px solid #0B5A7A;margin:10px 0;padding-top:8px;">
          <span style="font-weight:700;color:#0B5A7A;font-size:12px;">👥 السكان: ${fmt(d.population||0)}</span>
        </div>
        <div class="info-row"><span class="info-label">بطاقات إعاقة</span><span class="info-val">${fmt(d.disabled_cards||0)}</span></div>
        <div class="info-row"><span class="info-label">دور مسنين</span><span class="info-val">${fmt(d.elderly_homes||0)}</span></div>
        <div class="info-row"><span class="info-label">دور أيتام</span><span class="info-val">${fmt(d.orphan_homes||0)}</span></div>
        <div class="info-row"><span class="info-label">منظمات مسحوبة</span><span class="info-val">${fmt(d.ngo_active||0)}</span></div>
        <div class="info-row"><span class="info-label">مخالفات</span><span class="info-val">${fmt(d.violations||0)}</span></div>
        <div class="info-row"><span class="info-label">سلل غذائية</span><span class="info-val">${fmt(d.food_baskets||0)}</span></div>
        <div class="info-row"><span class="info-label">مخيمات</span><span class="info-val">${fmt(d.camps_count||0)}</span></div>
        <div class="info-row"><span class="info-label">مسجّلون تأمين</span><span class="info-val">${fmt(d.insurance_total||0)}</span></div>`;
    }
    popupContent += `</div>`;
...
function openGovProfile(gov){
  ...
  const title = document.getElementById('govProfileTitle');
  const body = document.getElementById('govProfileBody');
  if(!overlay || !title || !body) return;
  const arr = filtered().filter(d => d.gov === gov);
  if(arr.length === 0){
    body.innerHTML = '<p style="text-align:center;color:var(--ink-soft);padding:20px;">لا توجد بيانات لهذه المحافظة</p>';
  } else {
    const agg = {};
    const textFields = {};
    arr.forEach(d => {
    ...
    body.innerHTML = `
      <div class="profile-kpi-grid">
        <div class="profile-kpi"><div class="pk-val">${fmt(d.population||0)}</div><div class="pk-lbl">السكان</div></div>
        <div class="profile-kpi"><div class="pk-val">${fmt(d.males||0)}</div><div class="pk-lbl">الذكور</div></div>
        <div class="profile-kpi"><div class="pk-val">${fmt(d.females||0)}</div><div class="pk-lbl">الإناث</div></div>
        <div class="profile-kpi"><div class="pk-val">${fmt(d.employed||0)}</div><div class="pk-lbl">المشتغلون</div></div>
        <div class="profile-kpi"><div class="pk-val">${fmt(d.disabled_cards||0)}</div><div class="pk-lbl">بطاقات إعاقة</div></div>
        <div class="profile-kpi"><div class="pk-val">${fmt(d.food_baskets||0)}</div><div class="pk-lbl">سلل غذائية</div></div>
        <div class="profile-kpi"><div class="pk-val">${fmt(d.camps_count||0)}</div><div class="pk-lbl">المخيمات</div></div>
        <div class="profile-kpi"><div class="pk-val">${fmt(d.insurance_total||0)}</div><div class="pk-lbl">التأمينات</div></div>
      </div>
      <div class="profile-section-title">🏆 أهم الإنجازات</div>
      <div class="profile-text-block">${textFields.achievements ? textFields.achievements.join('<br>• ') : 'لم تُدرَج بيانات'}</div>
      <div class="profile-section-title">📌 أهم التوصيات</div>
      <div class="profile-text-block">${textFields.recommendations ? textFields.recommendations.join('<br>• ') : 'لم تُدرَج بيانات'}</div>
      <div class="profile-section-title">🧰 الاحتياجات الرئيسية</div>
      <div class="profile-text-block">${textFields.needs ? textFields.needs.join('<br>• ') : 'لم تُدرَج بيانات'}</div>
    `;
  }
  title.textContent = gov;
